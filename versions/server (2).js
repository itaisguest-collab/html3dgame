...
io.on('connection', socket => {
  ...
  socket.on('join_room', ({ code, name, playerName: nm }) => {
    ...
    if (!rooms[code]) rooms[code] = { name: name || code, code, players: {}, level: 1, wave: 1 };
    ...
    socket.emit('room_joined', { roomCode: code, others, level: rooms[code].level, wave: rooms[code].wave });
  ...
  // Co-op: relay enemy kill index so all clients remove it
  socket.on('enemy_kill', ({ idx }) => {
    if (!currentRoom) return;
    socket.to(currentRoom).emit('enemy_kill', { idx });
  });

  // Co-op: relay wave advance so all clients spawn same wave
  socket.on('wave_start', ({ level, wave }) => {
    if (!currentRoom) return;
    if (rooms[currentRoom]) { rooms[currentRoom].level = level; rooms[currentRoom].wave = wave; }
    socket.to(currentRoom).emit('wave_start', { level, wave });
  });
